...
Animate( )
{
	...
	uint64_t nt = currentTime;
	currentTime = timeSinceEpochMillisec();
	float dt = currentTime - nt;
	calculateAdvection(dt / 1000.f);
	glutSetWindow( MainWindow );
	glutPostRedisplay( );
}
...
InitLists( )
{
	...
	glNewList(BackgroundList, GL_COMPILE);
		...
		glVertex3f(-xPoint - CELLSIZE, -0.5, -xPoint - (2. * CELLSIZE));
		glVertex3f(xPoint + CELLSIZE, -0.5, -xPoint - (2. * CELLSIZE));
		glVertex3f(xPoint + CELLSIZE, -0.5, xPoint + 2. * CELLSIZE);
		glVertex3f(-xPoint - CELLSIZE, -0.5, xPoint + 2. * CELLSIZE);
		glEnd();
		...
		for (int row = 0; row < SIZE; row++) {
			for (int col = 0; col < SIZE; col++) {
				if (row == 0 || row == SIZE - 1 || col % (SIZE - 1) == 0) {
					...
					grid[SIZE * row + col].oldPressure = 0.;
					grid[SIZE * row + col].sourceTerm = 0.;
					...
					grid[SIZE * row + col].temperature = 0.;
					grid[SIZE * row + col].vx = -dy / r;
					grid[SIZE * row + col].vy = dx / r;
					grid[SIZE * row + col].pressure = 0.;
					grid[SIZE * row + col].density = 1.;
					grid[SIZE * row + col].oldPressure = 0.;
					grid[SIZE * row + col].sourceTerm = 0.;
